...
/// (#80). On a flaky radio (2016 Mac / OpenCore) the link dies the *instant* VWAR Loop Life arms that
...
/// connect and relies on the independent low-bandwidth 0x2A37 standard HR profile, which VWAR Loop Life already
...
struct BondRefusalGiveUp {
    ...
    static func pausedHint() -> String {
        "VWAR Loop Life stopped retrying because your strap keeps refusing to pair. It is likely still held by the official WHOOP app, or your phone is holding an old pairing. Close the WHOOP app, put the strap in pairing mode (tap until the LEDs flash blue), and if it is listed in your Bluetooth settings choose Forget This Device. Then tap Connect to try again."
    }
...
public final class BLEManager: NSObject, ObservableObject {
    ...
    private func connectCore(model: WhoopModel) {
        ...
        if let p = existing.first(where: { isPreferredPeripheral($0) }) {
            ...
            // platforms an LE link is shared system-wide, so a strap held by the WHOOP app, a prior VWAR Loop Life
            // session, or the OS itself reads .connected while VWAR Loop Life has NO session of its own yet. The old
    ...
    /// row alone left the strap connected — VWAR Loop Life kept re-grabbing it (the 3s disconnect→reconnect timer, the
    ...
    /// the fixed time — VWAR Loop Life has no light-sleep early-wake layer.
    ///
...
extension BLEManager: @preconcurrency CBCentralManagerDelegate {
    ...
    public func centralManager(_ central: CBCentralManager,
        ...
        // VWAR Loop Life itself works fine on the new firmware once the stale bond is cleared. (5/MG firmware reset, 2026-06)
        ...
        if let error = error {
            ...
            if selectedModel.deviceFamily == .whoop5, !didBond, insufficient {
                ...
                if bondGiveUp.gaveUp {
                    ...
                    state.pairingHint = "VWAR Loop Life can see your strap but it's refusing to pair - it's likely still bonded to the official WHOOP app, or your phone is holding an old pairing. To fix it: (1) fully close the WHOOP app, (2) on a 5.0/MG, tap the band repeatedly until the LEDs flash blue (pairing mode), (3) if your strap is listed under iPhone Settings → Bluetooth, tap it and choose Forget This Device, then reconnect in VWAR Loop Life."
