...
/// respiration ~5%; strain 0–21 from cardiovascular load). This is what makes VWAR Loop Life independent of
/// WHOOP's cloud , for any day the strap collected raw data with VWAR Loop Life connected, VWAR Loop Life scores it
...
final class IntelligenceEngine: ObservableObject {
    ...
    /// Who supplies the dashboard headline for a By-Day row. The By-Day card always shows VWAR Loop Life's OWN
    ...
    /// imported one, instead of always claiming "VWAR Loop Life-computed". (Sleep overhaul §2.6 honesty fix.)
    enum DaySource: Equatable {
        /// VWAR Loop Life scored this day itself from the raw strap streams; no import covers it.
        case computed
    ...
    /// over the whole asleep span. The mean always sits at-or-above the floor, so VWAR Loop Life reading lower is BY
    /// DESIGN, not a bug; logging both makes a "VWAR Loop Life RHR is lower than my other app" report explainable
    ...
    /// moved from the old 0–21 axis to VWAR Loop Life's own 0–100 axis. On-device computed rows since v2.6.1
    ...
    func analyzeRecent(maxDays: Int = 21, force: Bool = true) async {
        ...
        let scanned: [DayScan] = await Task.detached(priority: .utility) {
            ...
            for offset in 0..<maxDays {
                ...
                // MEAN over the whole asleep span. The mean always sits above the floor, so VWAR Loop Life looking
        ...
        // the computed row carries the VWAR Loop Life-derived Charge + the Rest composite. HONEST DATA: the engine
        ...
        results = out
        note = out.isEmpty
            ? "No scored nights yet. Wear the strap with VWAR Loop Life connected overnight and the engine will score your charge, effort and rest itself, no WHOOP cloud required."
            : nil
