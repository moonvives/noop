...
VWAR Loop Life Collector is the clean-room macOS application and command-line collector for the VWAR Loop Life /
G Band protocol.
It follows the useful architectural lesson from Goose — preserve raw transport evidence before parsing —
but contains no Goose code. Goose's Rust core declares itself `UNLICENSED`, so this project uses an original
...
Download `VWAR-Loop-Life-Collector.app.zip`, extract it, and move `VWAR Loop Life Collector.app` to Applications. The build
...
```text
Packages/VWARProtocol/.build/release/vwar-loop-life-capture
```
...
vwar-loop-life-capture \
  --name "Loop" \
  --duration 600 \
  --output ~/Documents/VWAR-Loop-Life-Capture
```

If the name is ambiguous, use the UUID printed by `--list`:

```bash
vwar-loop-life-capture --identifier 00000000-0000-0000-0000-000000000000 --duration 600
```
...
YYYYMMDD-HHMMSS-vwar-loop-life/
  vwar-transcript-private.json
  vwar-transcript-redacted.json
