...
struct TodayView: View {
    ...
    // `resolvedSeries` (the same imported-WHOOP > VWAR Loop Life-computed > Apple-Health precedence the dashboard
    ...
    // TODAY's in-progress Effort (VWAR Loop Life 0–100 axis), recomputed over the day's HR (local-midnight→now)
    ...
    /// the strap's real `deviceId`. The VWAR Loop Life-computed strap sibling (`deviceId + "-noop"`) reads
    ...
    // MARK: Apple Watch provenance (M1): "the watch is the sensor, VWAR Loop Life is the brain"
    ...
    var body: some View {
        ...
        .toolbar {
            ...
            ToolbarItem(placement: .navigation) {
                ...
                .help("Support VWAR Loop Life: help and contact")
                .accessibilityLabel("Support VWAR Loop Life: help and contact")
            }
    ...
    /// baseline (mirrors the recovery seed gate), the insight then falls back to the state word.
    ///
    /// STOPGAP (#696): VWAR Loop Life mixes HRV measurement methods on the shared `avgHrv` field,     /// strap/WHOOP-CSV HRV is RMSSD (~20-100 ms) while Apple-Health-imported HRV is SDNN
    ...
    /// The effective Effort strain (VWAR Loop Life 0–100 axis) the gauge shows. For TODAY this prefers the live
    ...
    private func effortStrain(_ d: DailyMetric?) -> Double? {
        #if DEBUG
        // DEBUG promo harness: pin Effort (VWAR Loop Life 0–100 axis) to the active frame's value. This single
    ...
    /// `strain` is on VWAR Loop Life's 0–100 Effort axis; `UnitFormatter.effortValue` converts it to the
    ...
    /// The Effort gauge's scale maximum, 100 on VWAR Loop Life's native axis, 21 on the WHOOP axis. Drives
    ...
    /// "Charge" marker (VWAR Loop Life's name for recovery) at wake time (sleep end), else at the window start.
    ...
    private var sourcesSummaryRow: some View {
        ...
        .accessibilityHint("Show what VWAR Loop Life is synced from")
    }
    ...
    /// strap doesn't transmit steps, so VWAR Loop Life estimates them from motion calibrated to the phone's count;
    ...
    private func loadDayScoped() async {
        ...
        // cross-source resolver applies the SAME imported-WHOOP > VWAR Loop Life-computed > Apple-Health precedence
