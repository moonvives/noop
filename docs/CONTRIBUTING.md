...
> hardware this software interoperates with. VWAR Loop Life reads **your own data** off **your own device**;
...
2. **Interoperability, not impersonation.** VWAR Loop Life talks to a strap the user already owns. It does not
...
├── Strand/                     # macOS SwiftUI app target (product name: VWAR Loop Life)
...
The scheme is `Strand`; the technical built product remains `NOOP.app` while its visible bundle name is `VWAR Loop Life`
(`project.yml` preserves `PRODUCT_NAME: NOOP`,
...
- **Licensing.** By opening a pull request you agree your contribution is licensed under the same
  [PolyForm Noncommercial License 1.0.0](../LICENSE) as the rest of VWAR Loop Life. Forks and personal,
...
VWAR Loop Life follows [Semantic Versioning](https://semver.org) — `MAJOR.MINOR.PATCH`:
...
VWAR Loop Life's logic already lives in cross-platform packages, so most platform work is app-layer wiring
...
- **Android (shipped).** A full, native Kotlin/Gradle client lives under `android/`, re-implementing
  ...
  [Releases](https://github.com/moonvives/noop/releases). Continued real-hardware testing across more devices is always welcome
...
- **iOS (build-from-source target on `main`).** iOS was folded into `main` in v1.94 as a first-class
  ...
  identity), which is at odds with VWAR Loop Life staying anonymous, so there are no pre-built downloads. Every
...
*VWAR Loop Life is an independent, unofficial, non-commercial interoperability project, not affiliated with,
