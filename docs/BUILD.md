...
> hardware this software interoperates with. VWAR Loop Life contains no WHOOP code, firmware, or assets. All
...
- The Xcode **scheme is `Strand`** and the technical product remains **`NOOP.app`** for compatibility.
  `project.yml` preserves `PRODUCT_NAME: NOOP` and bundle id `com.noopapp.noop`, while the visible
  display name is `VWAR Loop Life`.
...
### 3. The ad-hoc-signed technical bundle
...
Copy the bundle wherever you like (e.g. `~/Desktop/NOOP.app`) and double-click to launch. Finder and macOS
show its display name as VWAR Loop Life. On first
...
VWAR Loop Life connects over CoreBluetooth (`Strand/BLE/BLEManager.swift`). The central manager is created
on the main queue:
...
at odds with VWAR Loop Life staying anonymous, so the only way to run it is to build it yourself in Xcode.
...
(`VWAR-Loop-Life-vX.apk`) is published in [Releases](https://github.com/moonvives/noop/releases). A sample-data **demo** flavour still
...
VWAR Loop Life builds on prior community reverse-engineering and interoperability work:
